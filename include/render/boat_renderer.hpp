...
// format, MSAA material pipeline, and cascade shadow contract. One hull mesh
// (a shallow open box) is baked once and drawn per boat with a yaw rotation
// from its heading.
// ---------------------------------------------------------------------------
class BoatRenderer {
public:
    ...
    void renderShadowCasters(id<MTLRenderCommandEncoder> encoder, const std::vector<Boat>& boats,
                             const std::function<bool(const AABB&)>& isVisible);
