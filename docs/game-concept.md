...
- **Input:** pointer-locked mouse look that never lets the cursor escape mid-play, WASD aligned exactly with the camera, ESC always one keypress away from the pause menu.
...
Shipping: infinite terrain (10 biomes, caves, ores, trees, structures), block breaking/placing with persistence, day/night, weather particles, animals with state-machine AI and flocking, procedural audio, bloom + fog + clouds, the full menu suite.
