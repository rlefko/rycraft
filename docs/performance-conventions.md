...
- Player block edits flood their home cube plus at most three border face neighbors synchronously under the block-write lock, matching the generation-time precedent, so the render thread can never remesh an edited cube against stale packed light. Fluid writes stay on the deferred queue because their 1,024-cell tick cap would not fit synchronous floods. Edit-queued deep propagation drains ahead of the streaming reconcile queue, whose newest-first order would otherwise starve it during churn.
