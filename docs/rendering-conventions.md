...
- Cascaded visibility controls direct sun or moon only, then a sky-access cap (`smoothstep(0, 0.1, propagated skylight)`) removes direct light from cells that cannot see the sky. Why: inside cascade coverage the shadow map alone let a stray sun ray reach opaque-covered surfaces, so a sunbeam down a mine shaft cast harsh bands across the deep tunnel walls (and the screen-space bounce then relit those bands). Non-opaque cover keeps full skylight, so the cap never adds a second shadow under a leaf canopy. Propagated skylight and baked corner accessibility control ambient irradiance. Block light and emissive radiance remain independent and are never multiplied by skylight, baked accessibility, GTAO, or SSGI.
...
- The additive one-bounce is gated in the apply pass (only, so temporal history stays in un-gated units and needs no reset) by two factors: the receiver's baked sky access (`surface.a`, the same term the ambient-correction uses) so screen-space skylight cannot teleport into a sealed cave and follow the camera, and the day-night sky level (`filterParams.w`, 1 in daylight and ramping to 0 through twilight) so night auto-exposure cannot amplify the near-field bounce on open ground. A small floor keeps HDR emissive spill readable. The GTAO/ambient-correction term is already scaled by the night ambient and is left alone. Water tints fade by the same celestial signal so terrain indirect and night water darken together. Why: the additive bounce was the one indirect term not scaled by occlusion or light level, so it leaked a camera-following bright disk into dark tunnels and, at night, onto open ground, and the water tint's constant floor drew a ring.
