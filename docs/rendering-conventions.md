...
- **The interface side is a per-fragment geometric fact, and the Fresnel is exact.** The water pass classifies each fragment by the sign of dot(V, N) (an elevated lake's underside seen from dry land is still the water-to-air interface; the camera flag only decides which medium rays travel through) and evaluates the exact unpolarized dielectric Fresnel with total internal reflection falling out of Snell's law. Why: branching on the camera flag saturated dot(V, N) to zero for every submerged pixel, which read as past-critical total internal reflection everywhere and turned the whole surface into a permanent mirror, while Schlick approximations needed a hand-tuned ease at the critical angle that read as either a mushy window edge or hard mirror panels.
