...
- **From below, the Fresnel angle uses the flipped normal.** The interface faces a submerged camera as -N, and reflect() is symmetric in the normal's sign so only the angle needs it. Why: dot(V, N) against the up-facing wave normal saturated to zero for every submerged pixel, which read as past-critical-angle total internal reflection everywhere, and the entire surface became a permanent mirror that never transmitted.
