...
An immutable `LearnedAuthorityGraph` (`world/learned_authority_graph.hpp`) plans the unique coarse, Base, and Decoder windows a set of protected FINAL rectangles requires, computes their working-set retention bound before inference, and pins that closure in the backend's 384 MiB tensor-window store while the protected epoch is active. A pinned window is never evicted, so no window the active protected graph references is recomputed or evicted. The planner is a scheduling, dedup, and retention layer only: the window math stays in `InfiniteDiffusionBackend`, so authority bytes and qualification hashes are unchanged, and the plan's enumeration is cross-checked against the windows the backend actually computes. Protected owners group into one request only when the grouped rectangle costs strictly fewer unique windows than preparing them independently, not by bounding box. Why: overlapping cold-entry requests re-planned and recomputed the same windows, and the tensor cache could evict a window still needed by an in-flight protected build.
...
Render and fixed-tick threads must not perform or wait on model inference. `NativeHydrologyRouter` uses one process-wide admission gate for preview and final contexts. It admits at most 16 page builds, further limited by reported hardware concurrency and a one-GiB scratch reservation, rather than allowing each context to claim its own 16-build budget. The gate ranks waiters by the shared `AuthorityRequestPriority` and reserves lanes exactly as the learned queue does: distant `COARSE_PREVIEW` and `SPECULATIVE_PREFETCH` builds occupy at most half the lanes and visible-or-lower builds at most three quarters, so a distant owner can never occupy every lane while the player's exact band is unresolved. Why: strict-priority admission alone let distant builds that arrived first hold all 16 lanes until one finished, delaying the exact band by a full build. Generation failure is latched and user-visible. Existing resident geometry stays available, missing collision remains conservatively closed, and new generation does not publish an empty cube.
...
`common/trace.hpp` is one machine-readable critical-path trace across bootstrap, model windows, learned authority, hydrology, exact and far generation, upload, GPU residency, and visible screen-error debt. It is disabled by default and enabled with `RYCRAFT_TRACE=<path>`, which writes a Chrome Trace Event JSON plus a binary `.rytrace` at quiesce. Each 64-byte record carries the fingerprint, spatial key, camera epoch, quality, priority, dependency reason, execution time, retained bytes, and cancellation. The buffer is one up-front allocation that never grows: linear append drops past capacity and `RYCRAFT_TRACE_RING=1` overwrites the oldest. Disabled, an emit site is one relaxed atomic load and performs no heap allocation. The checked-in `rycraft_trace_summary` tool reports p50, p95, maximum, queue depth, cache reuse, cancellation, duplicate model windows, critical-path ownership, and omitted water or flora products from one trace. The subsystem is observability only and changes no authority, hydrology, selection, or rendering output. Why: later performance and quality work needs one reproducible attribution format instead of narrative timings.
