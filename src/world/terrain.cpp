...
double TerrainGenerator::generateBase(double x, double z, const TerrainConfig& config) const {
    double val = heightNoise_.octave2D(
...
double TerrainGenerator::generateRange(double x, double z, const TerrainConfig& config) const {
    ...
    double low = rangeLow_.octave2D(
        x * config.rangeFrequency,
        z * config.rangeFrequency,
        4,
        config.rangePersistence,
        config.lacunarity
    );
    low = (low + 1.0) * 0.5; // [0, 1]

    // High terrain field (mountains) — offset frequency to decorrelate
    double high = rangeHigh_.octave2D(
        x * config.rangeFrequency * 1.5,
        z * config.rangeFrequency * 1.5,
        4,
        config.rangePersistence,
        config.lacunarity
    );
    ...
    double selector = rangeSelector_.noise2D(
        x * config.rangeFrequency,
        z * config.rangeFrequency
    );
    // Map from [-1, 1] to [0, 1]
