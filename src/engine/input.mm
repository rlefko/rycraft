...
InputManager::InputManager(NSWindow* window)
    : state_{}, window_(window), lastMousePosition_{0, 0}, captured_(false), cursorHidden_(false),
      keyDownMonitor_(nil), keyUpMonitor_(nil), mouseMovedMonitor_(nil), mouseDraggedMonitor_(nil),
      mouseDownMonitor_(nil), mouseUpMonitor_(nil), scrollWheelMonitor_(nil) {
    ...
    keyDownMonitor_ = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyDown
                                                            handler:^NSEvent*(NSEvent* event) {
                                                                self->handleKeyDown(event);
                                                                return nil; // consume event
                                                            }];

    keyUpMonitor_ = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyUp
                                                          handler:^NSEvent*(NSEvent* event) {
                                                              self->handleKeyUp(event);
                                                              return nil;
                                                          }];

    mouseMovedMonitor_ = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskMouseMoved
                                                               handler:^NSEvent*(NSEvent* event) {
                                                                   self->handleMouseMoved(event);
                                                                   return event; // pass through
                                                               }];

    mouseDraggedMonitor_ =
    ...
    mouseUpMonitor_ = [NSEvent
        addLocalMonitorForEventsMatchingMask:NSEventMaskLeftMouseUp | NSEventMaskRightMouseUp |
    ...
    scrollWheelMonitor_ = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskScrollWheel
                                                                handler:^NSEvent*(NSEvent* event) {
...
InputManager::~InputManager() {
    ...
    if (keyDownMonitor_)
        [NSEvent removeMonitor:keyDownMonitor_];
    if (keyUpMonitor_)
        [NSEvent removeMonitor:keyUpMonitor_];
    if (mouseMovedMonitor_)
        [NSEvent removeMonitor:mouseMovedMonitor_];
    if (mouseDraggedMonitor_)
        [NSEvent removeMonitor:mouseDraggedMonitor_];
    if (mouseDownMonitor_)
        [NSEvent removeMonitor:mouseDownMonitor_];
    if (mouseUpMonitor_)
...
void InputManager::warpCursorToWindowCenter() {
    ...
    CGWarpMouseCursorPosition(CGPointMake(centerCocoa.x, screenHeight - centerCocoa.y));
