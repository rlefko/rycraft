...
- (BOOL)startWorldAtPath:(const std::string&)worldDir {
    ...
    if (state->generation.weather) {
        state->weatherSystem = std::make_unique<WeatherSystem>(state->world->generator());
        state->weatherSystem->requestSnapshot(static_cast<int64_t>(std::floor(spawnPos.x)),
                                              static_cast<int64_t>(std::floor(spawnPos.z)),
                                              state->worldTime);
        state->localWeather =
            state->weatherSystem->sample(spawnPos.x, spawnPos.z, state->worldTime);
        state->lastWeatherEventTick = state->worldTime;
        state->thunder.beginTimeline(state->worldTime);
...
- (void)render {
    ...
    _renderPipeline->render(
        ...
        &state->itemEntities.items(), &state->boats.boats(), state->weatherSnapshot,
        &state->lightningEvents);
